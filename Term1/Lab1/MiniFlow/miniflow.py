...
class Add(Neuron):
    def __init__(self, *inbound_neurons):
        ...
        Neuron.__init__(self, [item for item in inbound_neurons])
    ...
    def forward(self):
        ...
        Your code here!
        """
        self.value = sum([item.value for item in self.inbound_neurons])
...
class Linear(Neuron):
    ...
    def forward(self):
        ...
        Your code goes here!
        """
        listW = [W.value for W in self.weights]
        listX = [X.value for X in self.inbound_neurons]
        self.value = sum([W*X for W, X in zip(listW, listX)]) + self.bias.value
